...
Project and portfolio-area overview editors support rich text editing with selection-level formatting, links, images, videos, animations, formulas, tables, syntax-highlighted code blocks, hard line breaks, copy, paste, cut, and select all. When a rich field is active, a full-width editor toolbar appears with common document commands for formatting, alignment, links, images, videos, animations, tables, formulas, code blocks, image wrapping, and crop editing. The toolbar can be closed when it is not needed, and the same commands remain available from the shared right-click menu.
