...
4. Edit project overviews, sections, subsections, files, images, videos, animations, links, and appearance.
...
The builder uses a compact application menu bar instead of filling every window with more buttons. Use **File** for saving, printing, previews, publishing targets, updates, and applying a saved draft to the site. Use **Edit** for front-page/profile/fun-fact saves, copying selected project titles, clearing search, and refreshing the builder. Use **View** for category expansion, project navigation, previews, health reports, and zoom. Use **Project** for creating projects/categories, opening Overview or Compile Code, adding sections, cleaning empty placeholders, status visibility, and project deletion.

Use **Code** and **Build** for Compile Code workspace actions such as files, folders, imports, testbenches, Save Source, Save As, Beautify, line numbers, append-to-project, Compile, Build project, Run, Synthesize, Simulate, Scope, Syntax, Input, and launching Windows PowerShell at the project workspace. Open the **Analog/Mixed-Signal Design Lab** from the small `AM` button inside a project window when that hardware project needs schematic, IC, PCB, 3D physical planning, math, simulation, PDK, library, or Gerber groundwork. Use **Tools** for system readiness, security reports, compiler/toolchain checks, synthesis diagrams, and asset managers. Use **Preferences** for app viewing themes and Compile Code appearance themes. Use **Help** for customer-facing guide topics and keyboard shortcuts.
...
Inside a project window, each section and subsection now uses the same nested workspace pattern: controls first, **Overview** first, attached files as clean rows, and child subsections as vertical expandable rows. Expand a subsection to view and edit its own overview, add child subsections, attach files or images, or delete that subsection; collapse it again when you want a cleaner workspace. The right-click menu still understands the exact nested path, so you can open a section in a separate project window, copy its project path, duplicate it, move it earlier or later, delete it, or clean empty child placeholders. Up to eight project section tabs and up to eight detached project view windows can be open together, and each view keeps its own section/subsection location without changing the main project window.

Project section chips stay in the top navigation band. **Add section** appears as the final chip beside the existing project sections. Section and subsection actions are available from the active row instead of floating inside the content area. File evidence uses a plain full-width list where the filename is on the left and Delete stays at the far right. Empty placeholders can be cleaned from the current section/subsection through the right-click menu, or from the Compile Code **Project** menu for the whole current project. The project health report is available from the project-title right-click menu and the Compile Code **Project** menu; it summarizes parsed-preview state, overview readiness, attached evidence, empty placeholders, nested depth, source-file count, and status visibility before publishing.

## Text Editing

Project and portfolio-area overview editors support rich text editing with selection-level formatting, links, images, videos, animations, formulas, tables, syntax-highlighted code blocks, hard line breaks, copy, paste, cut, and select all. When a rich field is active, a compact editor toolbar appears at the top of the active window. Related commands are grouped under Text, Align, Insert, Image, Table, Block, and Clipboard; hover over or click a group to open its commands. The toolbar can be closed when it is not needed, double-clicking back inside the rich field brings it back, and the same commands remain available from the shared right-click menu.
...
Images inserted into rich fields can be resized from their edges, dragged to a new cursor position, cropped from the toolbar or right-click menu, given captions, and wrapped as block, inline, left, or right media. Image command buttons no longer sit on top of the image itself; click the image, then use the Image toolbar group or right-click menu. Dragging an image toward the left or right side of the editor automatically wraps nearby text around it, while dropping it near the middle keeps it on its own line unless it is small enough for inline placement. Videos and animations are added from the same builder workflow: choose a local MP4, WebM, OGG, MOV, GIF, APNG, WebP, or SVG animation file, or paste a YouTube, Vimeo, Google Drive, or direct media link. Choose **Show** when the media should play or animate inside the portfolio, or **Download only** when it should be available as evidence without occupying the page. Tables can be inserted from the toolbar or right-click menu by drawing a grid size or typing row and column counts, starting as small as 1 by 1. Click inside a table cell, then use the Table toolbar group or right-click menu to insert rows, insert columns, delete the current row, or delete the current column relative to the cursor cell. Links use a dedicated dialog for label, URL, and target behavior; saved links render as website links in project previews and published portfolios instead of being treated as downloadable files.
...
Projects include a separate **Compile Code** workspace by default. This is local builder tooling, not a required portfolio text block, and it can stay empty if a project does not need runnable source. Each project behaves like a small IDE workspace: the left explorer lists expandable nested folders and source files, clicking a file opens it as a closeable editor view, the main editor is the active syntax-highlighted typing surface, and the backend receives the whole workspace so files can include or call other files. Imported files and imported folders are added to Solution Explorer only; they do not open editor tabs until you click the specific file. Drag the divider beside Solution Explorer to widen the file tree when long folder or file names need more space. Ctrl-click files to select several files, then right-click one selected file and choose **Delete selected** to remove the group or **Move selected** to move the group into a project folder. Right-click a folder to delete the whole folder and files contained below it after confirmation. Right-click a folder or the workspace tree to create files inside that folder, create nested folders, import several files, import a whole directory, move selected source files, delete the selected file/folder, or open that exact workspace location in Windows File Explorer. The File menu also has **Open workspace in File Explorer** and **Reveal active source in File Explorer** for quick handoff to normal Windows file tools. The working-tree right-click menu has a small title grip so it can be dragged upward when it opens near the bottom of the screen and hides options. New files, testbenches, UVM-style testbenches, and imported files ask for language and file type before the file is registered in the workspace; imports can still use automatic language/type detection when that is the intended rule for a batch. Common keyboard editing works inside editable fields, including Ctrl+Z/Ctrl+Y undo/redo, Ctrl+C copy, Ctrl+V paste, right-click Paste, Ctrl+X cut, and Ctrl+S save. The code editor treats blank editor space as the active insertion area, so you can click an empty part of the editor and paste source code without first typing a character. Compiler checks and tool installation are intentionally kept in the **Tools** menu so the file tree context menu stays focused on file and folder work. JavaScript uses Node.js, Python uses the installed Python runtime, Java uses a JDK, C/C++ use a MinGW/WinLibs-style compiler when available, and Verilog/SystemVerilog use Icarus Verilog and vvp when installed.

Compile Code supports live syntax coloring and Beautify for C, C++, Verilog, SystemVerilog, LTspice, Java, JavaScript, Python, HTML, and support text files. Beautify formats the whole active file. The highlighter updates while typing, after files are imported, and after beautifying. While source is being typed, the builder waits briefly and runs a background syntax diagnostic pass against temporary compiler files. The dedicated **Syntax** panel shows live diagnostics with file, line, and character so syntax errors can be fixed before pressing Compile or Save Source. The active editor can show line numbers in a left gutter; toggle them from **Tools**, **Editor**, **Show/Hide line numbers**, or press Ctrl+Alt+L. The **Messages** panel stays reserved for save, compile, build, append, synthesis, simulation, and tool status events. This background check does not save the active file; **Save Source** and Ctrl+S remain the explicit write actions for the project workspace. Use **File details** only when you need the absolute path, file name, source language, HDL role, type, or save state; it opens as a moveable, resizable utility window instead of taking space from the editor. **Save Source As** creates an external copy through the system save picker or browser download flow, while the project workspace copy remains the compiler source of truth. Scrollable Compile Code areas are intentionally focused: click Solution Explorer, the editor, file tabs, Console, Messages, Terminal, Syntax, or Scope before using the mouse wheel in that specific area.

The Compile Code workspace has separate **Compile**, **Build project**, and **Run** controls. Verilog/SystemVerilog also show **Synthesize**, **Simulate**, and **Show scope**. Compile checks or builds the active source. Build project uses the files in the project workspace. Run executes the built active program when the language supports it. HDL work adds a tiny setup strip above the editor with **Design**, **Simulation**, and **Constraints** selectors so synthesis can target the active file, selected design files, or all design files; simulation can use a specific testbench top; and synthesis can record selected XDC/SDC/TCL constraint files. Use the small HDL strip control to hide or restore those settings when you need more editor space; collapsed mode still shows the design scope, simulation top, constraint count, and Nexys A7-100T target as compact chips. The default FPGA target is the Digilent Nexys A7-100T with the AMD/Xilinx Artix-7 `XC7A100T-1CSG324C` part. Synthesize runs the HDL synthesis path through Yosys from OSS CAD Suite when it is installed and stores synthesis artifacts beside the active source. When OSS CAD Suite is missing, the HDL tool installer can download the Windows x64 suite into the user-local Programs folder and the builder refreshes tool detection after extraction. A successful synthesis writes a Yosys netlist JSON file and a DOT graph artifact, then opens a large synthesized design schematic. The schematic places inputs on the left, outputs on the right, packs logic into balanced rows and columns, sizes cell blocks according to visible subcircuit complexity, and arrows nets in the direction of signal flow whenever the synthesized connection data makes that possible. Nets are drawn as straight or right-angled wires rather than curved graph lines. If Yosys is not installed, the viewer still displays a source-level module/instance graph from the current HDL workspace. Simulate runs HDL through vvp and can produce waveform data for the scope. SystemVerilog simulation is selected when files are marked SystemVerilog, use `.sv` or `.svh`, or contain SystemVerilog syntax such as `logic`, `always_ff`, interfaces, enums, packages, or assertions.

The synthesized design schematic includes explorer views for **Tree**, **Nets**, **Objects**, **Sources**, **IP blocks**, and **Timing**. The schematic window opens large, can be moved by dragging its title bar, and can be resized from any edge or corner while preserving the exact mouse offset so the window does not jump away from the pointer. Its control chrome is intentionally compact: the diagram receives most of the screen, the Fit command scales the generated graph into the visible canvas, wires are drawn as thin straight or right-angled directed nets, and expanded modules show input/inout ports on the left with outputs on the right. Double-click a module or synthesized cell to expand it like a lightweight Vivado-style hierarchy viewer. The first expansion shows ports, the second shows child blocks or leaf-gate information, and the third shows connected nets; one more double-click collapses the block again. Tree shows top-level inputs, logic objects, outputs, and bidirectional ports in a working-tree style. Nets shows driver/load relationships and net widths. Objects lists ports, primitives, and cells. Sources shows the file inventory used by synthesis; clicking a source opens metadata such as target language, HDL role, file type, simulator, synthesis tool, detected modules, line count, byte count, and path. IP blocks adds reusable SystemVerilog starters such as clock dividers, synchronizers, PWM, UART transmit, and register shells into the project workspace. Timing gives a structural timing review with register-like cell counts, memory-like cell counts, combinational cell counts, and notes explaining what still requires a real FPGA target and constraints.

The Compile Code workspace includes an explicit **Show output** control. Use it to jump directly to the **Console** output for the active source file. The workspace also has a **Messages** log for compile/save/append status entries and compiler progress, plus a **Terminal** tab that opens a real Windows PowerShell window from the selected project compile folder. The PowerShell window runs under the signed-in Windows user account, inherits the user environment plus detected compiler tool paths, and can navigate to normal Windows drives and folders that user can access. The builder does not simulate terminal input inside the web panel anymore; Terminal is now a launcher/status panel for the real shell. Use **Input** beside the output tabs to open a moveable, resizable Program input window for stdin before running code, keeping the main editor focused on source code. The output panel is locked into the workspace by default so it stays reachable while the editor scrolls; Console and Messages shrink to the content they actually need so empty black space does not take over the IDE, while Scope keeps a larger waveform viewport. Drag the output edge to resize the available output height, or unlock the panel when you want to float and resize it from every edge. The blue IDE status strip remains fixed at the bottom of the Compile Code workspace above Save / Save and close. **Append code to project** scans the project sections and nested subsections so the active source can be appended as a formatted code block to the correct existing overview without creating an extra section.
...
Verilog and SystemVerilog simulation requires a testbench. Mark HDL files as **Design** or **Testbench**, or use **Add testbench** to create a starter bench with `$dumpfile` and `$dumpvars`. HDL Compile can syntax-check/build design files, Synthesize can create a Yosys synthesis report and a readable synthesized schematic for design inspection, and Simulate requires the testbench and parses the generated VCD file into a **Signal scope** so signals can be viewed over time. The scope supports per-signal colors, per-signal radixes, signal filtering, time zoom, transition counts, and X/Z warnings. Icarus Verilog and vvp remain the required simulator tools; OSS CAD Suite supplies Yosys, Slang, GTKWave, and additional HDL utilities when installed, with Yosys used for synthesis and the builder's internal renderer used for zoomable netlist schematics.

## Analog/Mixed-Signal Design Lab

Each project can open a dedicated **AM Design Lab** from the small `AM` control in the project window. The lab is a full-screen project workspace for analog, mixed-signal, power, IC, and board-level design groundwork. It is separate from Compile Code: Compile Code is a source-file IDE, while AM Design Lab is the future schematic, simulation, layout, library, PDK, and manufacturing environment.

The first AM Lab release provides the GUI foundation and saved project state. It includes an SVG schematic canvas, project-specific component placement, direct component movement, rotate, mirror, duplicate, delete, wire drawing, labels, net names, selectable grid density, snap control, zoom, pan, fit view, design modes for IC/board/mixed work, a 3D physical-planning stage, and focus workflows for PLL, amplifier, and DC-to-DC conversion. Shortcuts include `V` select, `P` pan, `W` wire, `L` label, `N` net, `O` optimize, `R` resistor, `C` capacitor, `S` semiconductor, `D` diode, and `G` ground. `Ctrl` plus mouse wheel zooms the canvas, `Ctrl++` and `Ctrl+-` zoom by keyboard, `Ctrl+0` fits the view, arrow keys nudge the selected component, and `M` mirrors it.

The component library now renders EDA-style symbols instead of generic blocks wherever practical. It includes resistors, variable resistors, potentiometers, capacitors, polarized capacitors, variable capacitors, inductors, coupled inductors, transformers, ferrite beads, crystals, fuses, NMOS/PMOS variants, BJTs, JFETs, IGBTs, SCRs, TRIACs, diode families, LED/photodiode symbols, op amp/comparator triangles, differential pairs, current mirrors, PLL macros, sources, grounds, supplies, connectors, MCU/FPGA interfaces, and analog-front-end blocks. The library list shows live symbol thumbnails so selecting a part feels closer to an EDA component browser.

The AM stage row is organized around design stages instead of isolated notes: **Schematic**, **Symbol**, **Footprint**, **PCB**, **IC Layout**, **3D View**, **Math**, **Simulation**, **Gerber**, **PDK**, and **Libraries**. Stage changes preserve the same project context so a design can move from transistor-level or board-level schematic intent into symbols, footprints, physical planning, layout planning, equations, simulation plans, PDK setup, and manufacturing handoff. The bottom panel stores generated netlist scaffolds, simulation plans, math reports, layout notes, DRC/ERC reports, placed-object inventories, net inventories, source/evidence inventories, library-import plans, PDK notes, Gerber plans, and design notes.

The AM menus expose the deeper workflow without overcrowding the window. **Design** switches between IC transistor-level, board schematic, and mixed IC plus board design. **View** handles zoom, fit, grid, pan, and 3D physical-view planning. **Checks** creates lightweight DRC/ERC reports for missing references, missing values, duplicate reference designators, zero-length wires, unconnected schematic work, PDK readiness, and PCB readiness. **Analyze** calls the local AM backend analyzer, which inventories placed symbols, groups component categories, produces a starter SPICE-style netlist, and returns a backend readiness report through `/api/analog-mixed/analyze`. **Simulate** records future SPICE analysis plans such as operating point, DC sweep, AC, transient, noise, Monte Carlo, and corners. **Libraries** records local component/symbol/footprint/model work and future DigiKey or Mouser import hooks, and it can register starter device-level libraries for SKY130-style transistor cells, LTspice primitives, Texas Instruments power/interface shells, and KiCad PCB footprints. Imported devices become normal library items in that project: place them on the canvas, edit model/package/footprint/value parameters in the Inspector, include them in AM backend analysis, and use the KiCad template or Virtuoso-style flow actions to prepare board or IC handoff notes. **PDK** records generic, Sky130-style, GF180-style, and imported PDK planning. **Layout** tracks IC layout, PCB handoff, parasitic notes, matching notes, and Gerber-generation readiness.

This AM Lab is intentionally not advertised as a complete Cadence Virtuoso, LTspice, KiCad, or full SPICE replacement yet. It is the saved GUI and workflow layer those engines can plug into. Future versions can attach real SPICE solvers, netlist extraction, DRC/LVS engines, PCB backends, 3D package/board viewers, PDK importers, vendor API importers, optimization routines, and analog calculators behind the same user-facing stages.
...
Schematic evidence now receives a more deliberate viewer treatment. Schematic PDFs, schematic images, SVG diagrams, KiCad schematic files, KiCad PCB files, LTspice `.asc` files, SPICE decks, and netlists open in an enhanced schematic/document viewer when possible. The viewer includes Fit, Fit width, zoom, reset, and Contrast controls so circuit drawings can be inspected instead of appearing as cramped thumbnails. Text-based schematic files also get a lightweight generated map above the source when the builder can infer wires, symbols, and labels. Schematic-looking images embedded directly into rich overviews are displayed wider in project previews and in the published website so detailed analog circuits, PCB screenshots, and block diagrams remain legible.

Use **File > Print Portfolio** or Ctrl+P when you want a PDF-oriented copy of the portfolio. The print view is not a literal website screenshot; it reads the saved builder catalog and parsed project content, then opens a compact document layout with profile, contact links, public sections, projects, rich text, images, tables, links, and code blocks. Choose **Save as PDF** in the print dialog when a shareable document copy is needed.

The top application menu includes **Preferences**. Use Preferences to select light or dark builder skins, choose the Compile Code appearance, and decide whether the code editor shows line numbers. Each dropdown or checkbox repaints the visible builder immediately, so users can compare skins and editor density before closing the dialog. When dark mode is active, a **Day view** control appears in the main builder header so the app can be returned to the normal light editing view quickly. The main page does not expose a dark-mode shortcut; dark mode starts from Preferences.

Light and dark skins are viewing skins for the builder application. They recolor builder panels, dialogs, guide windows, menus, status chips, inputs, lists, editor frames, and window/icon controls so the application stays readable in different environments. Available app light skins include Default portfolio editing, Sky engineering, Paper schematic, Mint lab, Lavender signal, Rose review, Amber bench, and Slate drafting. Available app dark skins include Blueprint blue, Graphite bench, Emerald lab, Violet signal, Amber instrument, Teal oscilloscope, Crimson debug, and Steel workstation. Only **Light - Default portfolio editing** saves explicit text colors for the public website. Other light and dark skins are view-only: formatting controls still appear and preview color changes inside the builder, but saved website text colors normalize to the default public text color unless they were changed in the default light editor.
...
| `builder_polish_077` | Bumps the builder to `0.2.77`, polishes nested subsection navigation, adds section/subsection path and cleanup utilities to context menus, exposes whole-project empty-section cleanup from the Compile Code Project menu, and keeps file rows plain and template-independent. | `development` | `development` | Active |
| `builder_upgrade_078` | Bumps the builder to `0.2.78`, adds duplicate and move commands for project sections/subsections, adds a project health report for preview readiness and empty placeholders, and refreshes customer-facing guide text for the improved maintenance workflow. | `development` | `development` | Active |
| `builder_dark_windows_079` | Bumps the builder to `0.2.79`, adds selectable app-wide dark color skins, adds more Compile Code dark appearances, raises simultaneous project tabs/windows to eight each, and adds Open in new tab to the project section context menu. | `development` | `development` | Active |
| `theme_repaint_080` | Bumps the builder to `0.2.80`, makes Preferences dropdowns repaint immediately, connects dark skins to the actual builder chrome, and keeps Compile Code appearance changes visible across open workspaces. | `development` | `development` | Active |
| `subsection_vertical_081` | Bumps the builder to `0.2.81`, renders project subsections as vertical expandable rows with Overview first, keeps subsection delete controls visible, and syncs builder preview/public website parsing with the new nested layout. | `development` | `development` | Active |
| `appearance_modes_082` | Bumps the builder to `0.2.82`, adds multiple light viewing skins, keeps Light - Default portfolio editing as the only public text-color editing mode, normalizes view-only theme text colors before saving/publishing, and keeps formatting menus white and readable across themes. | `development` | `development` | Active |
| `terminal_behavior_083` | Bumps the builder to `0.2.83`, replaces the embedded Compile Code terminal prompt with a launcher for a real Windows PowerShell window at the project compile workspace, and documents the Console/Messages/Terminal split for users. | `development` | `development` | Active |
...
| `synthesis_schematic_viewer_095` | Bumps the builder to `0.2.94`, expands HDL synthesis into a full-screen readable schematic explorer with directional net arrows, source/nets/objects/tree/timing/IP views, source metadata windows, structural timing notes, and predesigned SystemVerilog IP block insertion. | `development` | `development` | Active |
| `synthesis_window_polish_096` | Bumps the builder to `0.2.95`, makes the synthesized schematic explorer a detailed moveable/resizable window, fixes title-bar dragging so the window follows the mouse without jumping, and standardizes small synthesis tabs/icons. | `development` | `development` | Active |
| `synthesis_orthogonal_drilldown_097` | Bumps the builder to `0.2.97`, changes synthesized schematics to straight/right-angled net routing, uses more balanced schematic space, adds double-click drill-down levels for modules and synthesized cells, makes SystemVerilog simulation selection content-aware, adds HDL Design/Simulation/Constraints selectors for individual-file synthesis and explicit testbench selection, defaults HDL target metadata to Nexys A7-100T, adds toggleable editor line numbers, improves dark Compile Code message/console contrast, and adds a compact app-style menu bar so extra commands live under File/Edit/View/Project/Code/Build/Tools/Preferences/Help instead of overcrowding project windows. | `development` | `development` | Active |
| `analog_mixed_workspace_098` | Bumps the builder to `0.2.99`, adds the first project-level Analog/Mixed-Signal Design Lab with an AM launcher, schematic canvas, component library, direct part movement, rotate/mirror/duplicate/delete controls, wire and label tools, zoom/pan/fit controls, IC/board/mixed modes, PLL/amplifier/DC-DC focus presets, design stages for schematic/symbol/footprint/PCB/IC layout/3D view/math/simulation/Gerber/PDK/libraries, DRC/ERC reports, netlist/math/layout/objects/nets/sources/library/PDK/Gerber planning panels, and customer-facing AM guide documentation. | `development` | `development` | Active |
| `advanced_am_eda_symbols_099` | Bumps the builder to `0.3.0`, expands the AM Lab with EDA-style symbol diagrams for passives, magnetics, diode families, MOSFETs, BJTs, JFETs, power devices, sources, references, and analog/mixed-signal macros, adds symbol thumbnails to the component library, and adds local AM backend endpoints for symbol inventory, netlist generation, and DRC/ERC-style analysis. | `development` | `development` | Active |
| `compile_header_compact_100` | Bumps the builder to `0.3.1`, shrinks the Compile Code HDL setup controls into a tiny hideable strip, keeps design/simulation/constraints/target summaries visible when collapsed, and preserves synthesis/simulation metadata without large panels. | `development` | `development` | Active |
| `am_eda_real_devices_101` | Bumps the builder to `0.3.2`, adds device-level AM imports for SKY130, LTspice, Texas Instruments shells, and KiCad PCB parts, exposes editable model, package, footprint, and parameter fields in the Inspector, adds KiCad PCB template and Virtuoso-style IC flow actions, mirrors imported devices in the AM backend analysis path, restores Compile Code File Explorer launch commands, and compacts the HDL schematic viewer with Fit zoom, port-aware module expansion, thinner directed nets, smaller schematic chrome, and more workspace for the actual netlist diagram. | `development` | `development` | Active |
