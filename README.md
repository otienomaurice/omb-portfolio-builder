...
The top application menu includes **Preferences**. Use Preferences to select light or dark builder skins and choose the Compile Code appearance. Each dropdown repaints the visible builder immediately, so users can compare skins before closing the dialog. When dark mode is active, a **Day view** control appears in the main builder header so the app can be returned to the normal light editing view quickly. The main page does not expose a dark-mode shortcut; dark mode starts from Preferences.
