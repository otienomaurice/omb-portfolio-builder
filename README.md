...
Compile Code supports syntax coloring and Beautify for C, C++, Verilog, SystemVerilog, LTspice, Java, JavaScript, Python, HTML, and support text files. Use **File details** only when you need to view or edit the absolute file path, source language, HDL role, or file title; otherwise keep it hidden so the editor keeps the maximum amount of space. Scrollable Compile Code areas are intentionally focused: click Solution Explorer, the editor, file tabs, Console, Messages, Terminal, or Scope before using the mouse wheel in that specific area.
