...
Project and portfolio-area overview editors support rich text editing with selection-level formatting, links, images, videos, animations, formulas, tables, syntax-highlighted code blocks, hard line breaks, copy, paste, cut, and select all. When a rich field is active, a compact editor toolbar appears at the top of the active window. Related commands are grouped under Text, Align, Insert, Image, Block, and Clipboard; hover over or click a group to open its commands. The toolbar can be closed when it is not needed, and the same commands remain available from the shared right-click menu.
