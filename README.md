...
The top application menu includes **Preferences**. Use Preferences to select dark builder mode, choose a dark color skin, and choose the Compile Code appearance. When dark mode is active, a **Day view** control appears in the main builder header so the app can be returned to the normal light editing view quickly. The main page does not expose a dark-mode shortcut; dark mode starts from Preferences.
