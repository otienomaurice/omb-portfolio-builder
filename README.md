...
Start with the curated file guides when you want to understand code like a book instead of a line-number report. The expanded server guide is now PDF-first and is generated by `docs/build_server_textbook_pdf.py`:
...
The server PDF begins with more than 50 filled pages of beginner JavaScript syntax, then explains `server.mjs` by user-facing backend features instead of by raw function order. The feature chapters cover local request boundaries, JSON responses, API routing, Save draft, Apply to site, uploads, Compile Code, GitHub authentication, Load from target, portfolio AI, updates, and security reporting without old reference-label blocks.
...
| `codex/server-pdf-js-textbook` | Rebuilds the `server.mjs` guide as a PDF-only textbook with a 50-plus-page JavaScript syntax primer and feature-based backend chapters without reference-label blocks. | `development` | `development` | Active |
