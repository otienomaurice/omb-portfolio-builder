...
Inside a project window, right-click a section or subsection and choose **Open in different tab** to keep several project views available at once. Nested subsections also expose edit/delete actions from their own context menu and from the current-section action row after you open them. Up to four project section tabs can be open together, and each tab keeps its own section/subsection location. The tab strip appears only when more than one project view is open, and the visible tabs show clean section titles instead of internal state labels.
...
The Compile Code workspace includes an explicit **Show output** control. Use it to jump directly to the **Console** output for the active source file. The workspace also has a **Messages** log for compile/save/append status entries and compiler progress, plus a terminal panel for running local PowerShell-style commands from the selected project compile folder. Type beside the prompt, press Enter, read the output directly below that command, and continue from the next prompt. Use **Input** beside the output tabs to open a moveable, resizable Program input window for stdin before running code, keeping the main editor focused on source code. The output panel is locked into the workspace by default so it stays reachable while the editor scrolls; drag its top edge to resize it, or unlock it only when you want to float and move the output panel. The blue IDE status strip remains fixed at the bottom of the output panel while Console, Messages, Terminal, or Scope are resized. **Append code to project** scans the project sections and nested subsections so the active source can be appended as a formatted code block to the correct existing overview without creating an extra section.
