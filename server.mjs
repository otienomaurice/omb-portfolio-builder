...
function analogMixedServerNetlist(projectTitle = "Project", workspaceInput = {}, options = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const pdkIncludes = options.includePdk ? analogMixedServerPdkIncludeLines(workspace, options.pdkStatus || {}) : [];
  const lines = [
    `* OMB AM backend netlist - ${projectTitle || "Project"}`,
    `* Mode: ${workspace.mode}; Focus: ${workspace.focus}`,
    `* PDK: ${workspace.activePdk || options.pdkStatus?.pdk?.variant || options.pdkStatus?.pdk?.family || "generic"}`,
    "* Generated by the local AM EDA analyzer. Review nodes and models before external SPICE simulation.",
    "",
    ...(pdkIncludes.length ? ["* PDK model includes", ...pdkIncludes, ""] : [])
  ];
...
const analogMixedPdkSkipFolders = new Set([".git", "node_modules", "cache", "tmp", "dist", "build", "__pycache__", ".venv", "venv"]);

async function analogMixedScanPdkFiles(folder, options = {}) {
  const rootFolder = path.resolve(String(folder || ""));
  const maxDepth = Number.isFinite(options.maxDepth) ? options.maxDepth : 7;
  const maxFiles = Number.isFinite(options.maxFiles) ? options.maxFiles : 1600;
  const folders = [];
  const files = [];
  const visit = async (current, depth = 0) => {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (analogMixedPdkSkipFolders.has(entry.name.toLowerCase())) continue;
  ...
  return { root: rootFolder, folders, files };
}

function analogMixedClassifyPdkFiles(scan = {}) {
  ...
  const libraries = unique([
    ...folders.filter((folder) => /libs\.(?:ref|tech)|cells?|models?|lef|gds|mag|ngspice|spectre|klayout|magic|netgen/i.test(folder)),
    ...layoutFiles.map((file) => path.dirname(file))
  ]);
  const corners = unique(modelFiles)
...
async function analogMixedInspectPdkFolder(pdkPath = "") {
  const requested = String(pdkPath || "").trim();
  if (!requested) throw new Error("A PDK folder path is required.");
  const resolved = path.resolve(requested.replace(/^["']|["']$/g, ""));
  let scan;
  try {
    scan = await analogMixedScanPdkFiles(resolved);
  } catch {
    throw new Error(`The PDK folder could not be read: ${resolved}`);
  }
  if (!scan.folders.length) throw new Error(`The PDK folder could not be read: ${resolved}`);
  const classified = analogMixedClassifyPdkFiles(scan);
  const descriptor = [resolved, ...classified.modelFiles, ...classified.techFiles, ...classified.libraries].join(" ").toLowerCase();
  const family = /sky130|skywater/.test(descriptor) ? "sky130"
    : /gf180|globalfoundries/.test(descriptor) ? "gf180"
      : /tsmc/.test(descriptor) ? "tsmc"
        : "custom-ic-pdk";
  const variant = /sky130b/.test(descriptor) ? "sky130B"
    : /sky130a/.test(descriptor) ? "sky130A"
      : /gf180mcu[a-z0-9_-]*/i.exec(descriptor)?.[0] || path.basename(resolved) || family;
  return {
    available: true,
    name: path.basename(resolved) || family,
    family,
    variant,
    path: resolved,
    source: "manual import",
    scannedFiles: scan.files.length,
    scannedFolders: scan.folders.length,
    ...classified,
    message: `${variant} PDK folder imported from ${resolved}.`
  };
}

function analogMixedServerPdkFromWorkspace(workspaceInput = {}, fallbackStatus = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  return workspace.stageData?.pdk?.importedPdk ||
    workspace.stageData?.["ic-layout"]?.pdkImport ||
    fallbackStatus.pdk ||
    null;
}

async function analogMixedPdkStatusForWorkspace(workspaceInput = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const imported = analogMixedServerPdkFromWorkspace(workspace);
  if (imported?.path) {
    try {
      return await analogMixedPdkStatus(imported.variant || imported.family || workspace.activePdk || "imported", imported.path);
    } catch (error) {
      const tools = {};
      for (const tool of ["ngspice", "xyce", "ltspice", "klayout", "magic", "netgen"]) tools[tool] = await findTool(tool);
      return {
        tools,
        pdk: {
          ...imported,
          available: false,
          message: error.message || "The previously imported PDK folder could not be read."
        }
      };
    }
  }
  return analogMixedPdkStatus(workspace.activePdk || "sky130");
}

function analogMixedServerPdkIncludeLines(workspaceInput = {}, pdkStatus = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const imported = analogMixedServerPdkFromWorkspace(workspace, pdkStatus);
  const files = [
    ...(workspace.stageData?.pdk?.modelFiles || []),
    ...(workspace.stageData?.["ic-layout"]?.pdkImport?.modelFiles || []),
    ...(imported?.modelFiles || []),
    ...(pdkStatus.pdk?.modelFiles || [])
  ];
  const seen = new Set();
  return files
    .filter(Boolean)
    ...
    .slice(0, 24)
    .map((file) => `.include "${file.replace(/\\/g, "/")}"`);
}

function analogMixedPdkTechFile(pdk = {}, tool = "magic") {
  const files = [...(pdk.techFiles || []), ...(pdk.layerFiles || [])];
  if (tool === "magic") return files.find((file) => /libs\.tech.*magic.*\.tech$/i.test(file)) || files.find((file) => /magic.*\.tech$/i.test(file)) || "";
  if (tool === "netgen") return files.find((file) => /netgen.*setup.*\.tcl$/i.test(file)) || files.find((file) => /setup.*\.tcl$/i.test(file)) || "";
  if (tool === "klayout") return files.find((file) => /klayout.*\.(?:lyp|lyt)$/i.test(file)) || files.find((file) => /\.(?:lyp|lyt)$/i.test(file)) || "";
  return "";
}

async function analogMixedPdkStatus(pdkName = "sky130", pdkPath = "") {
  const tools = {};
  for (const tool of ["ngspice", "xyce", "ltspice", "klayout", "magic", "netgen"]) {
    tools[tool] = await findTool(tool);
  }
  if (pdkPath) {
    const pdk = await analogMixedInspectPdkFolder(pdkPath);
    return { tools, pdk };
  }
  ...
  const techCandidates = [
    path.join(pdk.path, "libs.tech", "magic", "sky130A.tech"),
    path.join(pdk.path, "libs.tech", "netgen", "sky130A_setup.tcl"),
    path.join(pdk.path, "libs.tech", "klayout", "tech", "sky130A.lyt"),
    path.join(pdk.path, "libs.tech", "klayout", "sky130A.lyp")
  ];
  const techFiles = [];
  for (const techFile of techCandidates) {
    if (await pathExists(techFile)) techFiles.push(techFile);
  }
  return {
    tools,
    pdk: {
      ...
      modelFiles,
      techFiles,
      layerFiles: techFiles.filter((file) => /\.(?:lyp|lyt|map|tech|tf)$/i.test(file)),
      libraries,
...
function analogMixedServerProbeNode(workspaceInput = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const probe = workspace.probes.find((item) => String(item.label || "").trim());
  if (probe) return safeSegment(probe.label, "out").replace(/-/g, "_");
  const namedWire = workspace.wires.find((wire) => String(wire.name || "").trim() && !/^net_\d+$/i.test(wire.name));
  if (namedWire) return safeSegment(namedWire.name, "out").replace(/-/g, "_");
  return "out";
}
...
function analogMixedMagicExtractionScript(status = {}, gdsPath = "", pexNetlistPath = "", cellName = "am_top") {
  const pdk = status.pdk || {};
  const magicTech = analogMixedPdkTechFile(pdk, "magic");
  return [
    "# Auto-generated Magic extraction / PEX script",
    "# Run with: magic -dnull -noconsole <this-file>",
    magicTech ? `tech load ${magicTech.replace(/\\/g, "/")}` : "# Magic technology file not found in the imported PDK.",
    `gds read ${String(gdsPath || "").replace(/\\/g, "/")}`,
    `load ${String(cellName || "am_top").replace(/[^A-Za-z0-9_]/g, "_")}`,
    "select top cell",
    "extract all",
    "ext2spice lvs",
    "ext2spice cthresh 0",
    "ext2spice rthresh 0",
    `ext2spice -o ${String(pexNetlistPath || "extracted.spice").replace(/\\/g, "/")}`,
    "quit -noprompt"
  ].join("\n");
}

function analogMixedNetgenLvsScript(status = {}, netlistPath = "", pexNetlistPath = "", cellName = "am_top") {
  const pdk = status.pdk || {};
  const setup = analogMixedPdkTechFile(pdk, "netgen");
  const cleanCell = String(cellName || "am_top").replace(/[^A-Za-z0-9_]/g, "_");
  return [
    "# Auto-generated Netgen LVS script",
    setup ? `source ${setup.replace(/\\/g, "/")}` : "# Netgen setup file not found in the imported PDK.",
    `lvs {${String(netlistPath || "").replace(/\\/g, "/")} ${cleanCell}} {${String(pexNetlistPath || "").replace(/\\/g, "/")} ${cleanCell}} ${setup ? `{${setup.replace(/\\/g, "/")}}` : ""} lvs_report.txt -json`
  ].join("\n");
}

function analogMixedNoiseDeck(projectTitle = "Project", workspaceInput = {}, pdkStatus = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const baseNetlist = analogMixedServerNetlist(projectTitle, workspace, { includePdk: true, pdkStatus }).replace(/\n\.end\s*$/i, "");
  const outputNode = analogMixedServerProbeNode(workspace);
  const sourceRef = analogMixedServerInputSourceReference(workspace);
  return [
    `* OMB PDK-aware noise deck - ${projectTitle || "Project"}`,
    baseNetlist,
    "",
    `.noise V(${outputNode}) ${sourceRef} dec 20 1 1e9`,
    ".control",
    "set noaskquit",
    "run",
...
function analogMixedParasiticReport(projectTitle = "Project", status = {}, files = {}, toolRuns = []) {
  const pdk = status.pdk || {};
  return [
    `Parasitic extraction report for ${projectTitle || "Project"}`,
    "",
    `PDK: ${pdk.variant || pdk.family || pdk.name || "not selected"}`,
    `PDK path: ${pdk.path || "not imported"}`,
    `Magic script: ${files.magicExtractScriptPath || "not generated"}`,
    `PEX netlist target: ${files.pexNetlistPath || "not generated"}`,
    "",
    "Extraction method:",
    "Magic is used as the open-source extraction engine when it is installed and the imported PDK contains a Magic technology file. The script enables capacitance and resistance extraction thresholds so the generated SPICE netlist is prepared for post-layout simulation.",
    "",
    "Tool runs:",
    ...(toolRuns.length ? toolRuns.map((run) => `- ${run.tool}: ${run.ok ? "completed" : "not completed"}${run.exitCode !== undefined ? `, exit ${run.exitCode}` : ""}`) : ["- no extraction engine ran in this pass"])
  ].join("\n");
}

function analogMixedComplexAnalysisReport(projectTitle = "Project", workspaceInput = {}, status = {}, files = {}) {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const pdk = status.pdk || {};
  return [
    `Advanced IC analysis plan for ${projectTitle || "Project"}`,
    "",
    `PDK lane: ${pdk.variant || pdk.family || workspace.activePdk || "generic"}`,
    `Devices: ${workspace.components.length}`,
    `Schematic nets: ${workspace.wires.length}`,
    "",
    "Analysis methods prepared:",
    "- Operating-point sanity check for bias currents, node voltages, and region-of-operation assumptions.",
    "- DC sweep plan for transfer behavior, threshold points, regulation margin, and monotonicity.",
    "- AC/noise setup for bandwidth, gain/phase margin, input-referred noise, output noise, and contributor review.",
    "- Transient setup for startup, large-signal settling, slew, switching ripple, and sampled behavior.",
    "- PVT corner matrix using detected PDK corners when available.",
    "- Monte Carlo/mismatch planning for transistor W/L, multiplier, finger count, resistor ratio, and capacitor ratio sensitivity.",
    "- PEX handoff path using Magic extraction, Netgen LVS setup, and SPICE re-simulation from the extracted netlist.",
    "",
    "Generated handoff files:",
    ...Object.entries(files).filter(([key]) => key !== "runRoot").map(([key, value]) => `- ${key}: ${value}`)
  ].join("\n");
}
...
async function analogMixedBuildIcLayoutArtifacts(projectTitle = "Project", workspaceInput = {}, action = "layout-sync") {
  ...
  const magicExtractScriptPath = path.join(runRoot, `magic-pex-${stamp}.tcl`);
  const netgenScriptPath = path.join(runRoot, `netgen-lvs-${stamp}.tcl`);
  const netlistPath = path.join(runRoot, `schematic-${stamp}.spice`);
  const pexNetlistPath = path.join(runRoot, `extracted-pex-${stamp}.spice`);
  const noiseDeckPath = path.join(runRoot, `noise-${stamp}.cir`);
  ...
  await writeFile(magicScriptPath, [
    ...
    analogMixedPdkTechFile(status.pdk, "magic") ? `tech load ${analogMixedPdkTechFile(status.pdk, "magic").replace(/\\/g, "/")}` : "# Magic tech file not detected in the active PDK",
  ...
  await writeFile(magicExtractScriptPath, analogMixedMagicExtractionScript(status, gdsPath, pexNetlistPath, cellName), "utf8");
  await writeFile(netgenScriptPath, analogMixedNetgenLvsScript(status, netlistPath, pexNetlistPath, cellName), "utf8");
  await writeFile(noiseDeckPath, analogMixedNoiseDeck(projectTitle, workspace, status), "utf8");
  const toolRuns = [];
  ...
  const magic = status.tools?.magic;
  if (magic && ["ic-pex", "extract"].includes(action)) {
    const result = await runProcess(magic, ["-dnull", "-noconsole", magicExtractScriptPath], { cwd: runRoot, timeoutMs: 90000, env: await compileToolPathEnvironment() });
    toolRuns.push({ tool: "magic-pex", executable: magic, ok: result.ok, exitCode: result.code, stdout: result.stdout, stderr: result.stderr, elapsedMs: result.elapsedMs });
  }
  if (magic && action === "drc") {
    const result = await runProcess(magic, ["-dnull", "-noconsole", magicScriptPath], { cwd: runRoot, timeoutMs: 90000, env: await compileToolPathEnvironment() });
    toolRuns.push({ tool: "magic-drc", executable: magic, ok: result.ok, exitCode: result.code, stdout: result.stdout, stderr: result.stderr, elapsedMs: result.elapsedMs });
  }
  const ngspice = status.tools?.ngspice;
  if (ngspice && ["ic-noise-run", "ic-complex"].includes(action)) {
  ...
  if (!(await pathExists(noiseReportPath))) {
    await writeFile(noiseReportPath, [
      `Noise analysis report for ${projectTitle || "Project"}`,
      "",
      ngspice ? "ngspice did not produce a log file for this run." : "ngspice was not detected. The deck was generated and can be opened in any SPICE-compatible engine.",
      `Deck: ${noiseDeckPath}`,
      `Output node: ${analogMixedServerProbeNode(workspace)}`,
      `Input source: ${analogMixedServerInputSourceReference(workspace)}`
    ].join("\n"), "utf8");
  }
  if (!(await pathExists(pexNetlistPath))) {
    await writeFile(pexNetlistPath, [
      `* PEX netlist target for ${projectTitle || "Project"}`,
      "* Magic extraction has not produced a post-layout netlist yet.",
      "* Run the generated Magic PEX script after confirming the imported PDK tech file and GDS handoff.",
      ".end"
    ].join("\n"), "utf8");
  }
  ...
  const pdkName = status.pdk?.variant || status.pdk?.family || workspace.activePdk || "generic";
  const netlistRuns = [{ id: `netlist-${Date.now()}`, createdAt: new Date().toISOString(), status: "generated", pdk: pdkName, path: netlistPath, includes: analogMixedServerPdkIncludeLines(workspace, status).length }];
  ...
  const summary = {
    ...
    constraintGroups,
    signoffItems,
    pdkIncludes: analogMixedServerPdkIncludeLines(workspace, status),
    netlistRuns,
    parasiticRuns,
    noiseRuns,
    complexAnalysisRuns,
...
async function analogMixedSpiceDeck(projectTitle = "Project", workspaceInput = {}, kind = "op") {
  const workspace = normalizeAnalogMixedServerWorkspace(workspaceInput);
  const pdkStatus = analogMixedNeedsPdkModels(workspace) ? await analogMixedPdkStatusForWorkspace(workspace) : { pdk: null };
  const baseNetlist = analogMixedServerNetlist(projectTitle, workspace, { includePdk: true, pdkStatus }).replace(/\n\.end\s*$/i, "");
...
async function handleApi(request, response, url) {
  ...
  if (url.pathname === "/api/analog-mixed/netlist") {
    try {
      ...
      const pdkStatus = await analogMixedPdkStatusForWorkspace(workspace);
      const netlist = analogMixedServerNetlist(body.projectTitle || body.title || "Project", workspace, { includePdk: true, pdkStatus });
  ...
  if (url.pathname === "/api/analog-mixed/import-pdk") {
    try {
      const body = await readRequestJson(request);
      const pdk = await analogMixedInspectPdkFolder(body.pdkPath || body.path || "");
